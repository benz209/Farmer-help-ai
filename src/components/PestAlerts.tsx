  ...
  return (
    <div className="bg-white rounded-3xl p-6 shadow-xl border border-red-100">
      <div className="flex items-center gap-3 mb-6">
        ...
        <h3 className="text-xl font-bold text-gray-800">{getTranslation(language, 'pestAlerts')}</h3>
      </div>
      ...
      <div className="space-y-4">
        {alerts.map((alert, i) => (
          ...
          >
            <div className="mt-1">
              {alert.severity === 'high' ? <ShieldAlert className="w-5 h-5" /> : <Info className="w-5 h-5" />}
            </div>
            <div>
              <p className="font-bold text-sm uppercase tracking-wider mb-1">{alert.title}</p>
